...
FIXED_GREEN    = 42      # Fixed-Time 綠燈秒數（與 SUMO 預設號誌一致）
...
def get_avg_waiting():
    ...
    if not vehs:
        return 0.0
    total = sum(traci.vehicle.getAccumulatedWaitingTime(v) for v in vehs)
...
def evaluate_dqn(episodes):
    ...
    for ep in range(1, episodes + 1):
        ...
        green_time    = 0
        current_phase = 0
        vehicles_departed = set()   # 記錄進入模擬的所有車
        ep_avg_waits  = []
        ep_queues     = {"N":[], "E":[], "S":[], "W":[]}
        ...
        # 通過車數 = 進入模擬的車 - 模擬結束時還在路上的車（需在 close 前計算）
        vehicles_done = len(vehicles_departed) - traci.simulation.getMinExpectedNumber()
        ...
        all_throughput.append(int(vehicles_done))
