...
> 一個基於 Double DQN（Deep Q-Network）+ SUMO 模擬平台的智慧交通號誌控制研究專案

[![Python](https://img.shields.io/badge/Python-3.10-blue.svg)](https://www.python.org/)
[![PyTorch](https://img.shields.io/badge/PyTorch-2.11+-red.svg)](https://pytorch.org/)
[![SUMO](https://img.shields.io/badge/SUMO-1.26+-green.svg)](https://www.eclipse.org/sumo/)
...
本研究針對**現代都市交通號誌控制**的痛點，提出基於深度強化學習（Deep Reinforcement Learning, DRL）的動態路口決策系統。透過 SUMO 模擬平台與 Double DQN 演算法，讓 AI 號誌系統具備「全局視野」，自主學習最佳號誌切換策略。
...
| **D. Development 設計** | Based on Double DQN + SUMO 模擬平台 + TraCI API |
| **E. Experiments 實驗** | 於四向標準路口進行隨機車流壓力測試，5 局 × 3600 秒，與定時號誌比較 |
| **F. Findings 發現** | DQN 平均等待時間改善 **70.4%**（8.6s → 2.5s），佇列長度縮短 **56%** |

---

## 🏆 實驗結果 (Experimental Results)

### 訓練過程（500 集 Double DQN）
...
> **吞吐量差距說明**：DQN 積極切換導致黃燈損耗比例（21%）高於 Fixed-Time（9%），有效綠燈時間較少，屬 throughput-delay tradeoff 的預期現象。等待時間改善 70.4% 為本研究核心貢獻。
...
- **a₀ Keep**：維持當前相位，φₜ₊₁ = φₜ
- **a₁ Change**：切換下一相位（跳過 SUMO 黃燈相位，+2 mod num_phases）
- **安全約束**：最小綠燈時間 t_min = 10s、黃燈過渡 3s + 全紅 1s

### Reward Function (R)

```
rₜ = -Σ wₗ(t)   （對所有車道 ℓ ∈ {N, E, S, W} 求和）
```

- **wₗ(t)**：車道 ℓ 在時刻 t 的累積等待時間（getAccumulatedWaitingTime）
- **設計邏輯**：等待時間越長懲罰越大，Agent 自然學會優先疏散壅塞方向
...
yₜ = rₜ + γ · Q(sₜ₊₁, argmax_a Q(sₜ₊₁,a; θ); θ⁻)
```

- **θ**：Policy Network 權重（每步更新）
- **θ⁻**：Target Network 權重（每 500 步同步一次）
- **Double DQN 改進**：用 Policy Network 選動作、Target Network 算 Q 值，避免 Q 值高估
...
| Min green time | 10 秒 | 最小綠燈約束 |
| Decision step | 5 秒 | AI 決策間隔 |
| Fixed-Time 基準 | 42 秒 | SUMO netconvert 預設週期 |
...
│   ├── intersection.net.xml            # 完整路網（預設號誌 42 秒）
│   ├── intersection.rou.xml            # 車流設定（南北 0.3，東西 0.1）
...
# 訓練 Double DQN Agent（約 2-3 小時，500 集）
python train.py
...
1. 升級至 **Dueling DQN / Rainbow DQN**，改善 Q 值估計精確度
2. 改用 **Pressure-based Reward**（進出流量平衡），改善吞吐量 tradeoff
3. **Multi-Agent RL**：擴展至多路口協調控制
4. 接真實路口感測器（線圈/攝影機）驗證
